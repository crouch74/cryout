...
const registry = {
  get(id: string) {
    return getScenarioModule(id);
  },
  list() {
    return listScenarioModules();
  },
};

test('scenario registry exposes shipped scenarios and the native example scenario', () => {
  const ids = listScenarioMetadata().map((scenario) => scenario.id).sort();

  assert.deepEqual(ids, ['base_design', 'example_hooks', 'tahrir_square', 'woman_life_freedom']);
});
...
test('legacy-backed shipped scenarios create deterministic projected core states', () => {
  const summaries = Object.fromEntries(
    ['base_design', 'tahrir_square', 'woman_life_freedom'].map((scenarioId) => {
...
test('legacy-backed scenarios can dispatch through the command bridge', () => {
  const scenario = getScenarioModule('base_design');
  assert.ok(scenario);
...
test('example_hooks scenario exercises native hooks, cards, win checks, and system events', () => {
  const scenario = getScenarioModule('example_hooks');
  assert.ok(scenario);
  const harness = createScenarioHarness(scenario, { seed: 77 });
...
test('core saves round-trip through the scenario registry and run migrations', () => {
  const scenario = getScenarioModule('example_hooks');
  assert.ok(scenario);
  const state = createGameState(scenario, { seed: 88 });
  state.scenarioState.custom = { value: 1 } as unknown as import('../core/types.ts').JsonValue;
...
test('core invariant checks catch duplicate cards in a deck', () => {
  const scenario = getScenarioModule('example_hooks');
  assert.ok(scenario);
  const state = createGameState(scenario, { seed: 99 });
