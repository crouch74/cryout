// Deprecated compatibility surface. Prefer `src/engine/*` and `src/scenarios/*`.
  ...
  return Object.fromEntries(
    Object.entries(state.regions).map(([regionId, region]) => [
      regionId,
      {
        ...
        tags: [],
        data: {
          vulnerability: region.vulnerability as unknown as import('../../../types.ts').JsonValue,
        },
...
export function projectCompatStateToCore(
  ...
  return createBaseState({
    ...
    flags: {
      ...
      activeBeaconIds: legacyState.activeBeaconIds as unknown as import('../../../types.ts').JsonValue,
      lastSystemCardIds: legacyState.lastSystemCardIds as unknown as import('../../../types.ts').JsonValue,
      publicAttentionEvents: legacyState.publicAttentionEvents as unknown as import('../../../types.ts').JsonValue,
      terminalOutcome: legacyState.terminalOutcome as unknown as import('../../../types.ts').JsonValue,
    },
    counters: {
    ...
    scenarioState: {
      ...
      legacyState: legacyState as unknown as import('../../../types.ts').JsonValue,
    },
  });
}

export function projectCoreStateToCompat(state: CoreGameState): EngineState | null {
...
export function createCompatCommandBridge(): CommandBridge {
  return {
    createInitialState(scenario, options) {
      ...
      return projectCompatStateToCore(legacyState, scenario);
    },
    dispatch(state, command, scenario) {
      const legacyState = projectCoreStateToCompat(state);
          ...
          validationErrors: [
            {
              ...
              message: `Compatibility bridge cannot handle command ${command.type}.`,
            },
          ],
          debugTrace: [`🧱 Compatibility bridge rejected ${command.type}.`],
        };
      }
      ...
      return {
        ...
        emittedEvents,
        validationErrors: [],
        debugTrace: [`🧱 Compatibility bridge dispatched ${command.type} for ${rulesetId}.`],
      };
    },
  };
}

export function getCompatActionDefinitions(rulesetId: string): ActionDefinition[] {
  return compileContent(rulesetId).ruleset.actions;
}

export function serializeCompatReplayEnvelope(state: EngineState): SerializedGameEnvelope {
  const snapshot = serializeForReplay(state);
  return {
    coreVersion: CORE_VERSION,
    scenarioId: state.rulesetId,
    scenarioVersion: CORE_VERSION,
    state: projectCompatStateToCore(state, {
      metadata: {
        id: state.rulesetId,
...
export {
  ...
  createCompatCommandBridge as createLegacyCommandBridge,
  dispatchCommand,
  getCompatActionDefinitions as getLegacyActionDefinitions,
  getDisabledActionReason,
  initializeGame,
  normalizeEngineState,
  projectCompatStateToCore as projectLegacyStateToCore,
  projectCoreStateToCompat as projectCoreStateToLegacy,
  replayCommands,
  serializeCompatReplayEnvelope as serializeLegacyReplayEnvelope,
  serializeForReplay,
};
